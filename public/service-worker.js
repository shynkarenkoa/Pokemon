    ...
    function onClickMessage(event)
    {
        ...
        event.waitUntil(
            clients.matchAll({type: 'window'}).then(function(clientList)
            {
                ...
                {
                    console.log(event.notification.data.actions);
                    console.log(event.notification.data.actions[event.action]);

                    if(clients.openWindow && event.notification.data.actions[event.action].url)
